...
def get_partition_space(path_str):
    try:
        ...
        return f"Total: {t_gb:.1f} GB | Used: {u_gb:.1f} GB ({perc:.1f}%)"
    except: return "Space Unavailable"
...
# ===========================================================================
# --- CUSTOM UI WIDGETS (GANTT, PIE, LOG TAILER) ---
# ===========================================================================
...
class PDDashboard(QMainWindow):
    ...
    def init_ui(self):
        ...
        headers = ["Run Name", "RTL", "Src", "Status", "Stage", "User", "Size", "FM-N", "FM-U", "VSLP", "S-IR", "D-IR", "RunTime", "Start", "End", "Path", "Log", "UPF", "NONUPF", "VSLP_RPT", "STA", "IR", "Alias", "Starred_Hidden"]
    ...
    def refresh_view(self):
        ...
        for run in runs_to_process:
            ...
            if search_pattern != "**":
                ...
                combined = f"{run['r_name']} {run['rtl']} {notes}".lower()
        ...
        for run in normal_runs_list:
            blk_node = self._get_node(root, run["block"], "BLOCK")
            m_node   = self._get_node(blk_node, get_milestone(re.sub(r'_syn\d+$', '', run["rtl"])), "MILESTONE")
            rtl_node = self._get_node(m_node, run["rtl"], "RTL")
            
            item = CustomTreeItem(rtl_node)
            item.setText(0, run["r_name"]); item.setText(1, run["rtl"])
            item.setText(2, run["source"]); item.setText(3, run.get("fe_status", "COMPLETED"))
            item.setText(15, run["path"]);  item.setText(16, os.path.join(run["path"], "logs/compile_opt.log"))
            
            # Draw Status Colors
            if run.get("is_comp"): item.setForeground(3, QColor("#4CAF50"))
            elif run.get("fe_status") == "RUNNING": item.setForeground(3, QColor("#2196F3"))
            else: item.setForeground(3, QColor("#F44336"))
            
            # Stages
            if run["run_type"] == "BE":
                for stg in run["stages"]:
                    s_item = CustomTreeItem(item)
                    s_item.setText(0, stg["name"]); s_item.setData(0, Qt.UserRole, "STAGE")
                    s_item.setText(12, stg["info"]["runtime"])
                    s_item.setText(15, stg["stage_path"]); s_item.setText(16, stg["log"])

        # Update Pie Chart & Stars
        p = f = r = 0
        def update_nodes(node):
            ...
            if node.data(0, Qt.UserRole) not in ["BLOCK", "RTL", "MILESTONE"]:
                ...
                # Apply Golden Dot Icon if starred
                if node.text(15) in self.starred_runs:
                    node.setIcon(0, self.star_icon)
                    node.setData(0, Qt.UserRole + 5, True) # Hidden tag for sorting
                else:
