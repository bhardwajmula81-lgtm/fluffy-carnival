...
# ===========================================================================
def extract_fe_metrics(run_dir, source="WS", block=None, is_cancelled=None):
    """
    Extract all QoR metrics for a FE run.
    block: e.g. "BLK_ISP1" -- passed from dashboard tree.
    is_cancelled: optional callable() -> bool. Polled between sections;
                  on True, returns partial result with _cancelled=True.
    """
    ...
    _ck = is_cancelled if callable(is_cancelled) else (lambda: False)
    if _ck():
...
def extract_pnr_stage_metrics(run_dir, stage_name, source="WS", block=None,
                              stage_path=None, is_cancelled=None):
    """Extract QoR metrics for a single PNR stage.
    is_cancelled: optional callable() -> bool for early abort."""
