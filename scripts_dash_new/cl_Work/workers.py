...
# LRU-bounded path cache (M1) - prevents unbounded memory growth in long sessions
_path_cache      = OrderedDict()
_path_cache_lock = threading.Lock()
_PATH_CACHE_MAX  = 5000
_owner_cache      = OrderedDict()
_owner_cache_lock = threading.Lock()
_OWNER_CACHE_MAX = 2000

def cached_exists(path):
    with _path_cache_lock:
        if path in _path_cache:
            _path_cache.move_to_end(path)
...
def invalidate_path(path):
    """S5: drop a single path from cache so next check re-stats NFS."""
    if not path:
        return
    with _path_cache_lock:
...
# ---------------------------------------------------------------------------
# Workspace listdir cache (S1-WS) — eliminates repeated NFS directory listings
# during workspace discovery. Cleared at the start of every full scan.
# Thread-safe: multiple _scan_single_workspace threads share this cache.
# ---------------------------------------------------------------------------
_ws_ls_cache      = {}
_ws_ls_cache_lock = threading.Lock()
_WS_LS_CACHE_MAX  = 4000

def _ls_cached(path):
    """Return list of entry names for `path`, cached for the duration of a scan."""
    with _ws_ls_cache_lock:
        if path in _ws_ls_cache:
            return _ws_ls_cache[path]
    try:
        names = os.listdir(path)
    except OSError:
        names = []
    with _ws_ls_cache_lock:
        if len(_ws_ls_cache) < _WS_LS_CACHE_MAX:
            _ws_ls_cache[path] = names
    return names

def clear_ws_ls_cache():
    with _ws_ls_cache_lock:
        _ws_ls_cache.clear()
...
class ScannerWorker(QThread):
    ...
    def scan_ir_dir(self):
        ...
        for ir_base in ir_dirs:
            if not os.path.exists(ir_base):
                continue
            # S4: targeted recursive glob (much faster than os.walk on huge IR)
            try:
                log_paths = glob.glob(
                    os.path.join(ir_base, "**", "redhawk.log*"),
                    recursive=True)
            except Exception:
                log_paths = []
            for log_path in log_paths:
                f_name = os.path.basename(log_path)
    ...
    def _scan_single_workspace(self, ws_base, ws_name, tools_to_scan):
        ...
        current_rtl = "Unknown"
        # p4_sync scan: listdir once, filter in-memory (replaces glob "*.p4_sync")
        for n in _ls_cached(ws_path):
            if n.endswith('.p4_sync'):
                sf = os.path.join(ws_path, n)
                try:
        ...
        # IMPLEMENTATION/*/SOC/* — replaced 1 deep recursive glob with 3 shallow
        # listdir calls + in-memory filtering (S1-WS: ~20x fewer NFS calls).
        impl_path = os.path.join(ws_path, "IMPLEMENTATION")
        for impl_sub in _ls_cached(impl_path):
            soc_path = os.path.join(impl_path, impl_sub, "SOC")
            for ent_name in _ls_cached(soc_path):
                ent_path = os.path.join(soc_path, ent_name)
                is_fe_ws = (ws_base == _BASE_WS_FE())

                if is_fe_ws or "fc" in tools_to_scan:
                    fc_path = os.path.join(ent_path, "fc")
                    fc_entries = _ls_cached(fc_path)
                    if is_fe_ws:
                        for n in fc_entries:
                            if n.endswith('-FE'):
                                tasks.append((ent_name,
                                              os.path.join(fc_path, n),
                                              ws_path, current_rtl, "WS", "FE", None))
                    if "fc" in tools_to_scan:
                        for n in fc_entries:
                            if n.endswith('-BE'):
                                tasks.append((ent_name,
                                              os.path.join(fc_path, n),
                                              ws_path, current_rtl, "WS", "BE", None))

                if "innovus" in tools_to_scan:
                    inv_path = os.path.join(ent_path, "innovus")
                    for n in _ls_cached(inv_path):
                        if not n.startswith('.'):
                            rd = os.path.join(inv_path, n)
                            if os.path.isdir(rd):
                                tasks.append((ent_name, rd, ws_path,
                                              current_rtl, "WS", "BE", None))

        return tasks, releases_found

    # -----------------------------------------------------------------------
    # Outfeed discovery helper (parallel per block) -- S3
    # -----------------------------------------------------------------------
    def _scan_single_outfeed_block(self, ent_path, ent_name, tools_to_scan):
        """Outfeed structure:
           outfeed/{BLOCK}/{EVT_LABEL}/fc/{run}/{run}-FE
           outfeed/{BLOCK}/{EVT_LABEL}/fc/{run}-BE
           outfeed/{BLOCK}/{EVT_LABEL}/innovus/{run}[-BE]"""
        tasks = []
        # Outfeed: listdir + in-memory prefix filter replaces glob("EVT*")
        for n in _ls_cached(ent_path):
            if not n.startswith("EVT"):
                continue
            evt_dir  = os.path.join(ent_path, n)
            phys_evt = n
            blk_name = ent_name
            try:
                fc_path = os.path.join(evt_dir, "fc")
                # FE runs: outfeed/BLOCK/EVT/fc/{run_dir}/*-FE
                for run_dir in _ls_cached(fc_path):
                    run_path = os.path.join(fc_path, run_dir)
                    for fe_n in _ls_cached(run_path):
                        if fe_n.endswith('-FE'):
                            rd = os.path.join(run_path, fe_n)
                            tasks.append((blk_name, rd, rd, "UNKNOWN", "OUTFEED", "FE", phys_evt))
                if "fc" in tools_to_scan:
                    for fc_n in _ls_cached(fc_path):
                        if fc_n.endswith('-BE'):
                            rd = os.path.join(fc_path, fc_n)
                            tasks.append((blk_name, rd, rd, "UNKNOWN", "OUTFEED", "BE", phys_evt))
                if "innovus" in tools_to_scan:
                    inv_path = os.path.join(evt_dir, "innovus")
                    for inv_n in _ls_cached(inv_path):
                        rd = os.path.join(inv_path, inv_n)
                        if os.path.isdir(rd):
                            tasks.append((blk_name, rd, rd, "UNKNOWN", "OUTFEED", "BE", phys_evt))
            except Exception:
                continue
    ...
    def run(self):
        clear_path_cache()
        clear_ws_ls_cache()          # S1-WS: drop stale listdir cache from prior scan
        ...
        # --- Outfeed discovery (parallel per block) ---
        self.status_update.emit("Discovering OUTFEED directories...")
        if os.path.exists(_BASE_OUTFEED()):
            try:
                outfeed_blocks = [
                    (n, os.path.join(_BASE_OUTFEED(), n))
                    for n in os.listdir(_BASE_OUTFEED())
                    if os.path.isdir(os.path.join(_BASE_OUTFEED(), n))]
            except Exception:
                outfeed_blocks = []
            if outfeed_blocks:
                of_max_w = min(10, max(2, len(outfeed_blocks)))
                with concurrent.futures.ThreadPoolExecutor(
                        max_workers=of_max_w) as of_ex:
                    of_futures = [
                        of_ex.submit(
                            self._scan_single_outfeed_block,
                            ent_path, ent_name, tools_to_scan)
                        for ent_name, ent_path in outfeed_blocks]
                    for fut in concurrent.futures.as_completed(of_futures):
                        try:
...
class StageDetailWorker(QThread):
    # Emit be_run path (str) instead of raw QTreeWidgetItem to avoid
    # dangling-pointer crashes when the tree is rebuilt mid-flight.
    # Receiver looks up the live item via _signoff_items_by_path[path].
    finished = pyqtSignal(str, list)   # (be_run_path, enriched_stages)

    def __init__(self, be_run, be_item=None):
        super().__init__()
        self.be_run  = be_run
        # be_item kept as weak hint for compatibility; signal carries path only
        self.be_path = be_run.get("path", "") if isinstance(be_run, dict) else ""
    ...
    def run(self):
        ...
        for s in self.be_run.get("stages", []):
            ...
            s2["_lazy"]         = False
            enriched.append(s2)
        self.finished.emit(self.be_path, enriched)
...
class QuickStatusRefreshWorker(QThread):
    ...
    def run(self):
        # S5: Selective invalidation - drop only the paths we're re-checking
        # so the rest of the path cache stays warm.
        for task in self.tasks:
            run_path = task.get("path", "")
            if not run_path:
                continue
            invalidate_path(os.path.join(run_path, "pass", "compile_opt.pass"))
            invalidate_path(os.path.join(run_path, "logs", "compile_opt.log"))
            invalidate_path(os.path.join(run_path, "reports", "runtime.V2.rpt"))
        out = []
...
class MetricWorker(QThread):
    ...
    def run(self):
        ...
        if self._cancelled or self.isInterruptionRequested():
            self.finished.emit({})
            return
        # C6: pass cancellation callback into extractor for mid-flight abort
