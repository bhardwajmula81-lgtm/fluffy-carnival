...
from config import *
try:
    from config import _atomic_write_config
except Exception:
...
def get_user_email(username):
    ...
    try:
        timeout_sec = int(globals().get("USER_INFO_TIMEOUT_SEC", 5) or 5)
        res = subprocess.check_output(
            [USER_INFO_UTIL, '-a', username],
            stderr=subprocess.DEVNULL,
            timeout=timeout_sec).decode('utf-8', errors='ignore')
        fields = res.split(',')
        if len(fields) >= 9:
        ...
        debug_log("utilities: user_info timeout for {}".format(username), e)
    except Exception as e:
        debug_log("utilities: user_info lookup failed for {}".format(username), e)
    return ""
...
def save_mail_users_config(new_users):
    try:
        ...
        debug_log("utilities: save mail users failed", e)
