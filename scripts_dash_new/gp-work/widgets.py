...
class CustomTreeItem(QTreeWidgetItem):
    ...
    _RTL_RE = re.compile(r"EVT(\d+)_ML(\d+)_DEV(\d+)")
    ...
    def _date_sort_key(text):
        if not text or text in ("-", "N/A"):
            return None
        s = str(text)
        m = CustomTreeItem._DATE_TEXT_RE.search(s)
        if m:
        ...
        m = CustomTreeItem._DATE_NUM_RE.search(s)
        if m:
        ...
        try:
            if (self.data(0, Qt.UserRole) == "RTL"
                ...
                def rtl_key(item):
                    ...
                    if m:
                        return (0, int(m.group(1)), int(m.group(2)), int(m.group(3)), text)
                    nums = [int(x) for x in re.findall(r"\d+", text)]
                    return (1,) + tuple(nums[:6]) + (text,)
                return rtl_key(self) < rtl_key(other)
        except Exception:
            pass
